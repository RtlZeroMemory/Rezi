...
function renderDashboard(state: State): VNode {
  const visible = visibleServicesFor(state.services, state.filter, state.sort, state.sortDirection);
...
const DashboardStreamDriver = defineWidget<{ key?: string; state: State }>(
  (props, ctx) => {
    const telemetryStream = ctx.useMemo(() => createTelemetryStream(TELEMETRY_CADENCE_MS), []);
    ...
    ctx.useEffect(() => {
      ...
      app.update((state) => simulateTick(state, nextTick.nowMs));
    }, [telemetry.value]);

    return renderDashboard(props.state);
  },
  { name: "DashboardStreamDriver" },
);

app.view((state) =>
  DashboardStreamDriver({
    state,
  }),
);
