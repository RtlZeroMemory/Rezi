...
describe("DrawlistBuilder (ZRDL v1) - DRAW_TEXT_RUN", () => {
  test("emits blob span + DRAW_TEXT_RUN command referencing it", () => {
    ...
    assert.equal(u32(bytes, 32), 1); // strings_count (arena span only)
    assert.equal(u32(bytes, 36), 104); // strings_bytes_offset
    assert.equal(u32(bytes, 40), 8); // strings_bytes_len (4-byte aligned)
    assert.equal(u32(bytes, 44), 112); // blobs_span_offset
    assert.equal(u32(bytes, 48), 1); // blobs_count
    assert.equal(u32(bytes, 52), 120); // blobs_bytes_offset
    assert.equal(u32(bytes, 56), 84); // blobs_bytes_len
    ...
    // String spans: one arena entry at offset 96
    assert.equal(u32(bytes, 96 + 0), 0);
    assert.equal(u32(bytes, 96 + 4), 6);

    // String bytes: arena "ABCDEF" at offset 104 (padded to 4-byte alignment).
    assert.equal(String.fromCharCode(...bytes.subarray(104, 110)), "ABCDEF");

    // Blob span: single entry at offset 112
    assert.equal(u32(bytes, 112 + 0), 0);
    assert.equal(u32(bytes, 112 + 4), 84);

    // Blob bytes: seg_count=2 + two segments (40 bytes each)
    const blobOff = 120;
    ...
    assert.equal(u32(bytes, blobOff + 4 + 16), 0); // underline_rgb
    assert.equal(u32(bytes, blobOff + 4 + 20), 0); // link_uri_ref
    assert.equal(u32(bytes, blobOff + 4 + 24), 0); // link_id_ref
    assert.equal(u32(bytes, blobOff + 4 + 28), 0); // string_index
    assert.equal(u32(bytes, blobOff + 4 + 32), 0); // byte_off
    assert.equal(u32(bytes, blobOff + 4 + 36), 3); // byte_len

    // Segment 1
    const seg1 = blobOff + 4 + 40;
    ...
    assert.equal(u32(bytes, seg1 + 16), 0); // underline_rgb
    assert.equal(u32(bytes, seg1 + 20), 0); // link_uri_ref
    assert.equal(u32(bytes, seg1 + 24), 0); // link_id_ref
    assert.equal(u32(bytes, seg1 + 28), 0); // string_index
    assert.equal(u32(bytes, seg1 + 32), 3); // byte_off
    assert.equal(u32(bytes, seg1 + 36), 3); // byte_len
  });
});
