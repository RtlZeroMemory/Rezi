...
mod ffi {
    ...
    extern "C" {
        ...
        pub fn engine_poll_events(
            e: *mut zr_engine_t,
            timeout_ms: i32,
            out_buf: *mut u8,
            out_cap: i32,
        ) -> i32;
        pub fn engine_post_user_event(
            e: *mut zr_engine_t,
            tag: u32,
            payload: *const u8,
            payload_len: i32,
        ...
        pub fn engine_get_metrics(e: *mut zr_engine_t, out_metrics: *mut zr_metrics_t)
            -> ZrResultT;
        pub fn engine_get_caps(e: *mut zr_engine_t, out_caps: *mut zr_terminal_caps_t)
            -> ZrResultT;
...
fn apply_runtime_cfg_strict(
    dst: &mut ffi::zr_engine_runtime_config_t,
    obj: &JsObject,
) -> napi::Result<()> {
    validate_known_keys(obj, RUNTIME_CFG_KEYS, "engineSetConfig config")?;
    if let Some(lim) = js_obj(obj, "limits", "limits").map_err(|_| {
        Error::new(
            Status::InvalidArg,
            "engineSetConfig: limits must be an object",
        )
    })? {
    ...
    if let Some(plat) = js_obj(obj, "plat", "plat").map_err(|_| {
        Error::new(
            Status::InvalidArg,
            "engineSetConfig: plat must be an object",
        )
    })? {
...
pub fn engine_destroy(engine_id: u32) {
    ...
    let slot = lock_registry(|map| {
        ...
        if slot.owner_thread_id != current_thread_id() {
            return None;
        }
...
pub fn engine_get_metrics(engine_id: u32) -> napi::Result<EngineMetrics> {
    let guard = get_engine_guard(engine_id)
        .map_err(|_| Error::new(Status::InvalidArg, "ZR_ERR_INVALID_ARGUMENT"))?;
    if !guard.slot.is_owner_thread() {
...
pub fn engine_debug_query(
    ...
    Ok(DebugQueryResult {
        ...
        oldestRecordId: BigInt {
            sign_bit: false,
            words: vec![result.oldest_record_id],
        },
        newestRecordId: BigInt {
            sign_bit: false,
            words: vec![result.newest_record_id],
        },
...
pub fn engine_debug_get_stats(engine_id: u32) -> napi::Result<DebugStats> {
    ...
    Ok(DebugStats {
        totalRecords: BigInt {
            sign_bit: false,
            words: vec![stats.total_records],
        },
        totalDropped: BigInt {
            sign_bit: false,
            words: vec![stats.total_dropped],
        },
...
mod tests {
    ...
    impl TestFramebuffer {
        fn new(cols: u32, rows: u32) -> Self {
            ...
            let rc_clear =
                unsafe { ffi::zr_fb_clear(&mut raw as *mut _, &style_plain() as *const _) };
            assert_eq!(
                rc_clear,
                ffi::ZR_OK,
    ...
    fn fb_links_clone_from_failure_has_no_partial_effects() {
        ...
        let before_link_bytes = unsafe {
            std::slice::from_raw_parts(before_link_bytes_ptr, before_link_bytes_len as usize)
                .to_vec()
        };
        ...
        let clone_rc = unsafe {
            ffi::zr_fb_links_clone_from(&mut dst.raw as *mut _, &invalid_src as *const _)
        };
        ...
        let after_link_bytes = unsafe {
            std::slice::from_raw_parts(dst.raw.link_bytes, dst.raw.link_bytes_len as usize)
        };
