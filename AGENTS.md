...
4. **Composition + hook APIs.** `packages/core/src/widgets/composition.ts` contains `defineWidget` hooks, including `ctx.useReducer()` and animation hooks (`useTransition`, `useSpring`, `useSequence`, `useStagger`).
...
2. Create reducer in `helpers/state.ts` (or use `createSlice()` + `combineSlices()` for modular domains).
...
7. For derived app-state reads inside widgets, prefer memoized selectors via `createSelector(...)` with `ctx.useAppState(...)`.
...
3. `ctx.useReducer()`, `useTransition()/useSpring()/useSequence()/useStagger()`, `useTable()`, `useModalStack()`, `useForm()` — for complex interaction patterns.
...
3. **Calling hooks conditionally.** `defineWidget` hooks (`ctx.useState`, `ctx.useReducer`, `useTransition`, `useSpring`, `useSequence`, `useStagger`, `useAsync`, `useDebounce`, `usePrevious`, etc.) must be called in the same order every render. No conditional hook calls.
